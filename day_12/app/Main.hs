...
-- -- part1
-- findcomb:: ([[Char]], [Int]) -> Int
-- findcomb ([] , []) = 1
-- findcomb (_ , []) = 0
-- findcomb (lc:lcs, i:is)
--   | content == '.' = findcomb (lcs, i:is)
--   | content == '#' && sizeLc == i = findcomb lcs is
--   | content == '#' && sizeLc < i
--   | content == '#' && sizeLc < i
--     where
--       content = first lc
--       sizeLc = length lc
--       drop1 = findcomb (drop 1 lc) i:is
